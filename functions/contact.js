...
const CONTACT_US_EMAIL_NOTIFICATION = process.env.CONTACT_US_EMAIL_NOTIFICATION;
...
const saveUserContactMessage = async (db, { name, email, message }) => {
	console.log(`Saving contact message from ${email} with name ${name} and message ${message}`);
...
const sendEmail = async ({ name, email, message }) => {
	console.log(`Sending contact message received email from ${email} to ${CONTACT_US_EMAIL_NOTIFICATION}`);
	...
	console.log(`Email for contact message notification sent: ${JSON.stringify(result)}`);
}
...
module.exports.handler = async (event, context) => {
		...
		if (!name || !email || !message) {
			return { statusCode: 422, body: "Missing required params - name or email or message" };
		}
